...
### La red local: sin resolver, y el modo espejo descartado por experimento
...
Las tres salidas posibles, y dónde está cada una hoy:
...
devolver `192.168.1.227`, la IP de Windows. Pero los cuatro montajes del NAS dejaron de
funcionar — `ls /mnt/nas/musica` devolvía `No such device` tras 17 segundos, la firma del
automount agotando el `timeo=100,retrans=3` sin respuesta del NFS. El NAS estaba sano
(comprobado desde otro equipo: sirve NFS con normalidad). Detalle completo y vuelta atrás en
`PASOS.md` fase 8b.

**Y de paso corrige una suposición de este mismo plan.** Se escribió aquí que el NAT era lo que
impedía las callbacks de NFS, y que en modo espejo esa limitación *desaparecería*, abriendo la
puerta a `vers=4.1` con bloqueo. Es al revés: sin NAT el montaje ni siquiera se completa. Los
montajes se quedan con `vers=3,nolock` (`wsl/fstab.snippet`), y esa vía para mejorarlos queda
cerrada.

**Consecuencia:** el acceso desde la LAN sigue **sin resolver**. Quien lo retome debe saber que
lo que se juega es el NFS, que es de lo que come todo el stack: cualquier intento se hace con
el stack parado, con `verificar.sh` antes y después, y con la vuelta atrás preparada.
